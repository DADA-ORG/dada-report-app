...
router.get('/', async (req, res) => {
  try {
    ...
    const reports = records.map(r => ({
      ...
      calls_notes: r.fields['No of Calls & CDD N...'] || '',
      interviews: r.fields['No of itw today:'] || 0,
      sourcing_channel: r.fields['Channel of Sourcing...'] || '',
      final_update: r.fields['Final case update (if...'] || '',
...
router.post('/', async (req, res) => {
  try {
    ...
    if (calls_notes) fields['No of Calls & CDD N...'] = calls_notes;
    if (interviews !== undefined && interviews !== '') fields['No of itw today:'] = Number(interviews);
    if (sourcing_channel) fields['Channel of Sourcing...'] = sourcing_channel;
    if (final_update) fields['Final case update (if...'] = final_update;
